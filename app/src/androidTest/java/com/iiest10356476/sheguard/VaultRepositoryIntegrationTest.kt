...
class VaultRepositoryIntegrationTest {
    ...
    init {
        runBlocking {
            try {
                ...
                var result =
                    auth.signInWithEmailAndPassword("JoshuaPonquett@gmail.com", "TestAccount@123")
                        .await()
    ...
    fun uploadVault_withTempFile_uploadsSuccessfully() = runTest {
        ...
        try {
            ...
            uploadedFiles.plusAssign(vault.files)
            ...
            assertTrue(
                ...
                vault.files.first().url.startsWith("https://")
            )
        } finally {
    ...
    fun getVaultStatistics_countsCorrectly() = runTest {
        ...
        try {
            ...
            uploadedFiles.plusAssign(repo.uploadVault(photos = listOf(photoUri)).getOrThrow().files)

            // Upload video
            val videoFile = File.createTempFile("stat_video", ".mp4")
            videoFile.writeText("video content")
            val videoUri = Uri.fromFile(videoFile)
            uploadedFiles.plusAssign(repo.uploadVault(videos = listOf(videoUri)).getOrThrow().files)
